...
"""
hugging face 가입 - 
Transformer 모델이 무료로 올라와요
내 인공지능 모델들 1GB 이하는 여기에 올리면되요

streamlit 가입 -
원래는 AI모델을 돌릴려면 20만원짜리 서버를 개인이 구입하거나
아니면, 여기 학원에 cpu 서버가 있긴 해요
학원 서버는... 내가 새로운 파인튜닝 프로젝트 만들었을때
올리기가 애매해요

하지만, streamlit + hugging 를 이용하면
무료로 체험판 사이트를 만들수 있어요

NCS 과정 끝나고, 우리만에 AI 만들때는 이런 과정으로 갈거에요
모델을 훈련시킬때는 구글꺼, 테스트할때는
로컬에서 cpu로 하고
그다음에 코드를 개조하고(GPT 한테 부탁)
streamlit 에 올릴거에요
"""